...
const AddBooksModal = ({ onClose, shelfId, onBooksAdded }) => {
  ...
  const handleSubmitImage = async (e) => {
    ...
    try {
      const detectResponse = await axios.post('http://localhost:5000/book/detect', formData);
      const bookTitles = detectResponse.data.bookTitles;

      const metadataResponse = await axios.post('http://localhost:5000/book/metadata', {
        bookTitles: bookTitles.map(book => book.bookTitle)
      });
  ...
  const handleManualSearch = async (e) => {
    ...
    setIsSearching(true);
    try {
      const response = await axios.post('http://localhost:5000/book/metadata', {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/book/batch', formattedBooks);
